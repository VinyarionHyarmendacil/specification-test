import { state, unstableValues } from "./_cache.js";
import { failUnauthenticated, test, testOperation } from "./_utilities.js";

test.before(failUnauthenticated);

const customInstance = state.get("blackcat-custom-instance");
const currentUser = state.get("current-user");
const messageSlot = 0;
const messageType = "message";
const messageContents = "Hello, World! " + currentUser.last_activity;
const messageContentsRateLimited = "I am speed!" + currentUser.last_activity;

test.todo("Invite User");

test(
	testOperation,
	"inviteMyselfTo",
	{
		parameters: {
			worldId: customInstance.worldId,
			instanceId: customInstance.instanceId
		},
		statusCode: 200,
		unstable: true
	}
);

test.todo("Request Invite");
test.todo("Respond Invite");

test(
	testOperation,
	"getInviteMessages",
	{
		parameters: {
			userId: currentUser.id,
			messageType: messageType
		},
		statusCode: 200,
		unstable: true
	}
);

test.serial(
	testOperation,
	"getInviteMessage",
	{
		parameters: {
			userId: currentUser.id,
			messageType: messageType,
			slot: messageSlot
		},
		statusCode: 200,
		unstable: [ "updatedAt" ]
	},
	(t) => {
		const { body } = t.context;
		
		t.is(
			body.canBeUpdated,
			true,
			"Message slot 0 should be updatable"
		);
		t.is(
			body.remainingCooldownMinutes,
			0,
			"Message slot 0 should be have 0 minutes of cooldown"
		);
	}
);

test.serial(
	"message slot 0 reset",
	testOperation,
	"resetInviteMessage",
	{
		parameters: {
			userId: currentUser.id,
			messageType: messageType,
			slot: messageSlot
		},
		statusCode: 200,
		unstable: true
	},
	(t) => {
		const { body } = t.context;
		
		t.is(
			body[messageSlot].canBeUpdated,
			true,
			"Message slot 0 should be updatable after being reset"
		);
		t.is(
			body[messageSlot].remainingCooldownMinutes > 0,
			true,
			"Message slot 0 should be have greater than 0 minutes of cooldown"
		);
	}
);

test.serial(
	"message slot 0 reset rate limited",
	testOperation,
	"resetInviteMessage",
	{
		parameters: {
			userId: currentUser.id,
			messageType: messageType,
			slot: messageSlot
		},
		statusCode: 429,
		unstable: false
	}
);

test.serial(
	"message slot 0 update",
	testOperation,
	"updateInviteMessage",
	{
		requestBody: {
			message: messageContents
		},
		parameters: {
			userId: currentUser.id,
			messageType: messageType,
			slot: messageSlot
		},
		statusCode: 200,
		unstable: true
	},
	(t) => {
		const { body } = t.context;
		
		t.is(
			body[messageSlot].canBeUpdated,
			false,
			"Message slot 0 should not be updatable after being updated"
		);
		t.is(
			body[messageSlot].remainingCooldownMinutes > 0,
			true,
			"Message slot 0 should be have greater than 0 minutes of cooldown"
		);
	}
);

test.serial(
	"message slot 0 update rate limited",
	testOperation,
	"updateInviteMessage",
	{
		requestBody: {
			message: messageContentsRateLimited
		},
		parameters: {
			userId: currentUser.id,
			messageType: messageType,
			slot: messageSlot
		},
		statusCode: 429,
		unstable: false
	}
);
